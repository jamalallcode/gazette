      ...
      <div className="bg-white py-[6px] px-4 border-b border-zinc-200 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div 
              onMouseEnter={() => setBellDropdownOpen(true)}
              onMouseLeave={() => setBellDropdownOpen(false)}
              ...
              id="realtime-order-notifications-bell"
            >
              <div className="relative p-2 rounded-full hover:bg-zinc-100 transition duration-150 animate-in zoom-in-95">
                ...
                {unreadNotifications.length > 0 && (
                  <span className="absolute -top-1 -right-1 bg-red-600 text-white font-extrabold text-[9px] h-4 w-4 rounded-full flex items-center justify-center shadow animate-pulse">
              ...
              <div className="text-left hidden xs:block leading-none">
                <span className="block text-xs font-black text-zinc-900 font-sans group-hover:text-[#f58220] transition">
                  {language === 'bn' ? "নোটিফিকেশন" : "Alerts"}
                </span>
                {unreadNotifications.length > 0 && (
                  <span className="text-[9.5px] text-red-600 font-extrabold mt-0.5 animate-pulse block">
                    {language === 'bn' ? `${unreadNotifications.length}টি নতুন অর্ডার` : `${unreadNotifications.length} New Orders`}
                  </span>
                )}
              </div>
              ...
              {bellDropdownOpen && (
                <div 
                  className="absolute top-full right-0 pt-2 w-80 h-auto z-[999]"
                    ...
                    <div className="flex justify-between items-center pb-2 border-b border-zinc-100">
                      ...
                      {unreadNotifications.length > 0 && (
                        <button 
                          ...
                          className="bg-transparent border-0 text-[10px] text-zinc-400 hover:text-[#f58220] font-black uppercase cursor-pointer"
                        >
                      ...
                      {unreadNotifications.length === 0 ? (
                        <div className="py-6 text-center space-y-1.5 text-zinc-400">
                          ...
                          <p className="text-[9px] text-zinc-400 max-w-[200px] mx-auto leading-relaxed">
                            {language === 'bn' ? "গ্রাহকরা অর্ডার করলে এইখানে সাথে সাথে অ্যালার্ট বাজবে।" : "New client orders will trigger live auditory alert bells here."}
                          </p>
                        </div>
                      ) : (
                            ...
                            <div key={order.id} className="bg-amber-50/40 hover:bg-amber-50 border border-amber-100/55 rounded-xl p-3 space-y-2.5 transition">
                              ...
                              <div className="flex justify-between items-center text-[10.5px]">
                                ...
                                <div className="flex items-center gap-1.5 font-bold">
                                  ...
                                  <a 
                                    href={`https://wa.me/${order.customerInfo.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Assalamu Alaikum, I am the admin of Nabik Bazar. I received your order ${order.id} and am checking to confirm it!`)}`}
                                    target="_blank"
                                  ...
                                  {/* Direct invoice rasiid viewing event */}
                                  <button
                                    onClick={() => {
                                      // Trigger invoice print fallback
                                      const printBtn = document.getElementById("print-invoice-receipt-btn");
                                      if (printBtn) {
                                        printBtn.click();
                                      } else {
                                        window.dispatchEvent(new CustomEvent("app-toast", { detail: `Invoice is ready inside the success voucher!` }));
                                      }
                                    }}
